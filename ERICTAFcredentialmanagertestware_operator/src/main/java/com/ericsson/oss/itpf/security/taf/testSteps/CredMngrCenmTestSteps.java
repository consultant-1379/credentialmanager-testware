 ...
 * </ul>
 */
public class CredMngrCenmTestSteps {
    ...
    public static final String TAF1_GROUP = "cENMfiles/TAF1";
    public static final long CLITOOL_TIMEOUT = 40;

    public static final String FILE_VERSION_PARAM = "starSearchFileParam";
    
    private static List<String> LOCAL_FILE_NAMES = Arrays.asList("configTAF.sh", "executeTest.sh", "runTAF.sh", "secretTAF.yaml", "xmlTAF.xml");
    private static final Logger log = LoggerFactory.getLogger(CredMngrCenmTestSteps.class);
    ...
    private String lastRemoteFileWithPath;
    
    @TestStep(id = DELETE_DIR_TESTSTEP)
    public void deleteDirTestStep(@Input(FILE_VERSION_PARAM) final String fileVersion) {
        final String rmdirStringCommand = String.format("rm -rf %s", fileVersion);
        final String lsStringCommand = String.format("ls -la %s", fileVersion);
        ...
        log.info("CredMngrCenmTestSteps: deleteDirTestStep" + lsResult);
    }

    @TestStep(id = CREATE_DIR_TESTSTEP)
    public void createDirTestStep(@Input(FILE_VERSION_PARAM) final String fileVersion) {
        log.info("CredMngrCenmTestSteps: Director used: " + pibHost);
        final String mkdirStringCommand = String.format("mkdir -p %s", fileVersion);
        final String lsStringCommand = String.format("ls -la %s", fileVersion);
        final String chmodStringCommand = String.format("chmod -R 777 %s", fileVersion);
        ...
        final CliCommandResult lsResult = shellPibConn.execute(lsStringCommand);
        log.info("CREATE_DIR_TESTSTEP : CredMngrCenmTestSteps: createDirTestStep" + lsResult);
    }

    @TestStep(id = COPY_FILES_TESTSTEP)
    public void copyFilesTestStep(@Input(FILE_VERSION_PARAM) final String fileVersion) {
        log.info("COPY_FILES_TESTSTEP : You are using file from : " + fileVersion);
        ...
        log.info("COPY_FILES_TESTSTEP : CredMngrCenmTestSteps: copyFilesTestStep pem file path -> " + pemKeyFile);
        for (final String fileName : LOCAL_FILE_NAMES) {
            log.info("COPY_FILES_TESTSTEP : LOCAL_FILE_NAMES is :  -> " + fileName);
            final String remoteFileName = String.format("%s/%s", fileVersion, fileName);
            final String remoteFullPath = String.format("%s/%s", tempRemoteFileWithPath, remoteFileName);
            final boolean copyResult = remote.copyLocalFileToRemoteWithSshKeyFile(fileName, remoteFullPath, fileVersion, pemKeyFile);
            log.info("COPY_FILES_TESTSTEP : fileName  is  :" + fileName);
            log.info("COPY_FILES_TESTSTEP : remoteFullPath  is  :" + remoteFullPath);
            log.info("COPY_FILES_TESTSTEP : fileVersion  is  :" + fileVersion);
            log.info("COPY_FILES_TESTSTEP : pemKeyFile  is  :" + pemKeyFile);
//             final boolean copyResult2 = remote.copyLocalDirToRemoteWithSshKeyFile(fileVersion, remoteFullPath, pemKeyFile);
//             Assertions.assertThat(copyResult2).as(String.format("Transfer dir %s failed", remoteFileName)).isTrue();
        ...
        final CliCommandResult lsResult = shellPibConn.execute(lsStringCommand);
        log.info("COPY_FILES_TESTSTEP : CredMngrCenmTestSteps: copyFilesTestStep" + lsResult);
    }

    @TestStep(id = RUN_TEST_TESTSTEP)
    public void runTestTestStep(@Input(FILE_VERSION_PARAM) final String fileVersion) {
        log.info("RUN_TEST_TESTSTEP : You are using file from : " + TAF1_GROUP);
        tempRemoteFileWithPath = shellPibConn.execute("pwd").getOutput();
        log.info("RUN_TEST_TESTSTEP : Base Path is :" + tempRemoteFileWithPath);
        final String cdCommand = String.format("cd %s/%s", tempRemoteFileWithPath, TAF1_GROUP);
        shellPibConn.execute(cdCommand);
        lastRemoteFileWithPath = shellPibConn.execute("pwd").getOutput();
        log.info("RUN_TEST_TESTSTEP : RunTest Path is :" + lastRemoteFileWithPath);
